...
const AddReview = () => {
  ...
  useEffect(() => {
    axios(
      `https://backend-ashinteriorbd-8a6e29b4adf0.herokuapp.com/userReview?email=${email}`
    ).then((res) => {
  ...
  const handleDelete = (id) => {
    ...
    swal({
      ...
      if (wantDelete) {
        ...
        axios
          .delete(
            `https://backend-ashinteriorbd-8a6e29b4adf0.herokuapp.com/deleteReview/${id}`
          )
          .then((res) => {
      ...
      {description ? (
        <div className="userReviewBox">
          <div className="review col-md-6 mx-auto">
            {img ? <img src={img} alt="" /> : <img src={`${userImg}`} alt="" />}
