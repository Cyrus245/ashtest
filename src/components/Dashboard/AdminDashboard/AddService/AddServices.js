...
const AddService = ({ edit, setEdit, services }) => {
  ...
  const onSubmit = (data) => {
    ...
    if (edit) {
      axios
        .put(
          `https://backend-ashinteriorbd-8a6e29b4adf0.herokuapp.com/api/product/v1/product/update/${edit}`,
          serviceInfo
        )
        .then((res) => {
      ...
      axios
        .post(
          "https://backend-ashinteriorbd-8a6e29b4adf0.herokuapp.com/api/product/v1/product/add",
          serviceInfo
        )
        .then((res) => {
          toast.dismiss(loading);
