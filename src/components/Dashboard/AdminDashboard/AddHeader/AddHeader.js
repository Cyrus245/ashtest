...
const AddHeader = ({ edit, setEdit, headers }) => {
  ...
  const onSubmit = (data) => {
    ...
    if (edit) {
      axios
        .put(
          `https://backend-ashinteriorbd-8a6e29b4adf0.herokuapp.com/api/content/v1/contentheader/update/${edit}`,
          headerInfo
        )
        .then((res) => {
      ...
      axios
        .post(
          "https://backend-ashinteriorbd-8a6e29b4adf0.herokuapp.com/api/content/v1/contentheader/add",
          headerInfo
        )
        .then((res) => {
          toast.dismiss(loading);
